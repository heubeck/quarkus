...
=== Customizing RBAC (Cluster)Roles and bindings

Especially for building Kubernetes operators, additional RBAC configuration may be required. The Kubernetes extension got you covered with the `quarkus.kubernetes.policies` config group:

[source,properties]
----
# Role and RoleBinding resources are generated and bound to the desired app kind (Deployment or StatefulSet)
quarkus.kubernetes.policies.ext.api-groups=extensions,apps <1>
quarkus.kubernetes.policies.ext.resources=deployments,deployments/status
quarkus.kubernetes.policies.ext.verbs=get,watch,list
quarkus.kubernetes.policies.srv.resources=services <2>
quarkus.kubernetes.policies.srv.verbs=get,update

# ClusterRole and ClusterBinding are generated for policy groups defined as 'cluster-wide'
quarkus.kubernetes.policies.crd.cluster-wide=true <3>
quarkus.kubernetes.policies.crd.api-groups=apiextensions.k8s.io
quarkus.kubernetes.policies.crd.resources=customresourcedefinitions
...
<2> The grouping key `srv` describes another `PolicyRule` for the same `Role` - all policies of namespaced Roles are merged together (see below).
<3> Policy groups marked as `cluster-wide` will be generated as `ClusterRole` with related `ClusterRoleBinding`. Like with Roles, all `cluster-wide` policies are merged in a single `ClusterRole`.

The configuration example will be generated to manifests like this:
...
kind: ClusterRole
metadata:
  name: awesome-project-cluster-role
rules:
  - apiGroups:
...
<1> This service account is bound to the generated app manifest (Deployment or StatefulSet).
...
Specifically Quarkus implements the link:https://github.com/k8s-service-bindings/spec#workload-projection[Workload Projection] part of the specification, thus allowing
...
=== Generating ServiceBinding resources
The <<service_binding, Service Binding>> covers, how the application will consume the bound service. 
However, we still haven't touched on how the actual binding happens. The binding is the responsibility of the Service Binding Operator. The user provides a `ServiceBinding` resource which expresses which application will bound to which services and the operator is responsible for mounting the required files to container.

A `ServiceBinding` looks like:
...
The `quarkus-kubernetes-service-binding` extension provides a more compact way of expressing this information:
...
By adding the above configuration properties inside your `application.properties` the `quarkus-kubernetes` in cooperation with the `quarkus-kubernetes-service-binding` extension will generate the `ServiceBinding` for you.
And this is something that is meant to work regardles of the kubernets flavour you are using, so it's expected to also work for Openshift and Knative, without any additional action from the user.

In the configuration above `db-demo` has a double role. It is used to correlate / group `api-version` and `kind` together, but it's also used as the `name` of the custom resource. The later can be overriden if needed. For example:
...
NOTE: Advanced, features (e.g. `ServiceBiniding` mappings) are not yet supported.

Here is the list of: link:https://github.com/redhat-developer/service-binding-operator#known-bindable-operators[Known bindable Operators].


[TIP]
====
For more details on how the discovery of the available properties works, please read the link:https://github.com/k8s-service-bindings/spec#workload-projection[Workload Projection] part of the Service Binding specification.
====
